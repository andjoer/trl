...
class FluxFillPipeline(
    ...
    def __call__(
        ...
        if masked_image_latents is not None:
            ...
            print(f"input shape of masked image latents before concat {masked_image_latents.size()}")
            print(f"input shape of maskt {mask.size()}")
            masked_image_latents = torch.cat((masked_image_latents, mask), dim=-1)
            print(f"input shape of masked image latents after concat {masked_image_latents.size()}")
        # 6. Prepare timesteps
